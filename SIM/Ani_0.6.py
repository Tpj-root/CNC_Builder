...
stroke_length_value = tk.IntVar(value=100)  # Stroke length variable
...
def update(positions):
    # Adjust bar height based on stroke length
    current_stroke_length = stroke_length_value.get() / scaling_factor
    for bar, y in zip(bars, positions):
        bar.set_height(current_stroke_length)
        bar.set_y(y)
    return bars
